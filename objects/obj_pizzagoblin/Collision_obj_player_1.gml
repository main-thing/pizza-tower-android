stunned = 200
if (place_meeting(x, y, obj_player) && state == 82 && obj_player.state == states.normal && obj_player.xscale == sign((x - other.x)) && floor(obj_player.y) == floor(y) && (obj_player.x <= (x - 20) || obj_player.x >= (x + 20)))
{
...
else
{
    with (obj_player)
    {
        ...
        if ((state == states.mach2 || state == states.mach3 || state == states.machroll) && place_meeting(x, (y + 1), obj_collisionparent) && other.flying == 0)
        {
            if (x != other.x)
                other.image_xscale = (-(sign((other.x - x))))
