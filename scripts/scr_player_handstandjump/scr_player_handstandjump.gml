function scr_player_handstandjump() {
	...
	if (grounded && sprite_index == airattackdash && key_attack && character != "N" && global.attackstyle != 2) state = states.mach2
	if (floor(image_index) == (image_number - 1) && sprite_index == attackdash) state = states.normal
	if (floor(image_index) == (image_number - 1) && sprite_index == airattackdashstart) sprite_index = airattackdash
